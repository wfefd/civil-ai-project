...
금오공과대학교 학사 민원에서 반복적으로 발생하는 문의를 효율적으로 처리하기 위한 **RAG 기반 AI 답변 지원 시스템**입니다.
...
AI 서버는 Python 패키지를 `requirements.txt`로 관리합니다.
...
> Gemini API 연동 코드에서 `google-genai` 패키지를 사용하고 있다면 `google-generativeai` 대신 `google-genai`를 사용해야 합니다.
...
> `qna-cookie`는 QnA 게시판 접근이 필요한 경우 설정합니다. 개인 로그인 쿠키나 API 키는 GitHub에 올리지 않아야 합니다.
...
공지사항 본문 `content`를 chunk 단위로 나누어 저장합니다.
